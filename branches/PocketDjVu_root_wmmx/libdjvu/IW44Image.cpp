...
mmx_bv_1 ( short* &q, short* e, int s, int s3 )
{
  ...
  while (q+3 < e)
  {
    ...
    MMXar( paddd,      d16,mm0);
    MMXar( paddd,      d16,mm1);
    MMXrr( psubd,      mm2,mm0);  // MM0=[ (c1+b1)*9-a1-d1+16, ... // SIV: +8 was corrected as +16.
    MMXrr( psubd,      mm3,mm1);  // MM1=[ (c3+b3)*9-a3-d3+16, ... // SIV: +8 was corrected as +16.
    MMXir( psrad,      5,mm0);
...
mmx_bv_1( short* &q, short* e, int s, int s3 )
{
  ...
  //*/
  register __m64 w1  = _mm_set1_pi16( 1 );
  register __m64 w9  = _mm_set1_pi16( 9 );
  register __m64 d16 = _mm_set1_pi32( 16 );

  while (q+3 < e)
  { 
    {      
      // b = [ b3, b2, b1, b0 ]
      register __m64 b = _mm_setr_pi16( *(q-s), *(q-s+1), *(q-s+2), *(q-s+3) );
      // t = [ b3, b2, b1, b0 ]
      register __m64 t = b;
      // c = [ c3, c2, c1, c0 ]
      register __m64 c = _mm_setr_pi16( *(q+s), *(q+s+1), *(q+s+2), *(q+s+3) );      
      // b = [ c1, b1, c0, b0 ]
      b = _m_punpcklwd( c, b );
      // t = [ c3, b3, c2, b2 ]
      t = _m_punpckhwd( c, t );
      ...
      //.......................
      // c = [ a3, a2, a1, a0 ]
      c = _mm_setr_pi16( *(q-s3), *(q-s3+1), *(q-s3+2), *(q-s3+3) );
      // e = [ a3, a2, a1, a0 ]
      register __m64 e = c;
      // d = [ d3, d2, d1, d0 ]
      register __m64 d = _mm_setr_pi16( *(q+s3), *(q+s3+1), *(q+s3+2), *(q+s3+3) );
      ...
      //......................
      // b = [ (c1+b1)*9+16, (c0+b0)*9+16 ]
      b = _m_paddd( b, d16 );
      // t = [ (c3+b3)*9+16, (c2+b2)*9+16 ]
      t = _m_paddd( t, d16 );

      // b = [ (c1+b1)*9+16 - d1-a1, (c0+b0)*9+16 - d0-a0 ]
      b = _m_psubd( b, d );
      // t = [ (c3+b3)*9+16 - d3-a3, (c2+b2)*9+16 - d2-a2]
      t = _m_psubd( t, e );

      //.......................
      // b = [ ((c1+b1)*9+16 - d1-a1)>>5, ((c0+b0)*9+16 - d0-a0)>>5 ]
      b = _m_psradi( b, 5 );
      // c = [ q3, q2, q1, q0 ]
      c = _mm_setr_pi16( *(q), *(q+1), *(q+2), *(q+3) );
      // t = [ ((c3+b3)*9+16 - d3-a3)>>5, ((c2+b2)*9+16 - d2-a2)>>5 ]
      t = _m_psradi( t, 5 );
      // b = [ x3, x2, x1, x0 ] = 
      //     [ ((c3+b3)*9+16 - d3-a3)>>5, ((c2+b2)*9+16 - d2-a2)>>5, ((c1+b1)*9+16 - d1-a1)>>5, ((c0+b0)*9+16 - d0-a0)>>5 ]
      b = _m_packssdw( b, t );
      // c = [ q3-x3, q2-x2, q1-x1, q0-x0 ]
      c = _m_psubw( c, b );
      //.......................
      *(__int64*)q = _m_cvt_pi2si( c );
    ...
    // 1-Lifting
    {
      ...
      if (y>=3 && y+3<h)
      {
        ...
        if (scale==1 && MMXControl::mmxflag>0)
          mmx_bv_1(q, e, s, s3);
#endif
        // SIV: TODO : add checking if WMMX is enabled
        //if (scale==1)
        //  mmx_bv_1(q, e, s, s3);
