...
#if defined(WIN32) || defined(OS2) || defined(UNDER_CE)
...
#elif defined(WIN32) || defined(UNDER_CE)// (either Windows or WCE)
    GUTF8String filename(UTF8Filename());
    if(filename.length() >= MAX_PATH)
      {
        if(!filename.cmp("\\\\",2))
...
GURL::mkdir() const
{
  ...
  if(!retval)
    ...
    GPBuffer<wchar_t> gwfilename(wfilename,wfilename_size);
    filename.ncopy(wfilename,wfilename_size);
	 retval = CreateDirectoryW(wfilename,NULL);
#else
# error "Define something here for your operating system"
#endif
    }
...
GURL::deletefile(void) const
{
  ...
  if(is_local_file_url())
    ...
    filename.ncopy(wfilename,wfilename_size);
	 if (is_dir())
		 retval = RemoveDirectoryW(wfilename);
	 else
		 retval = DeleteFileW(wfilename);
#else
# error "Define something here for your operating system"
#endif
  }
...
#ifdef UNICODE
    wchar_t buf[1024];
	 MultiByteToWideChar(CP_UTF8,0,wildcard.UTF8Filename(),-1,buf,1024);
...
#ifdef UNICODE
			char buf[4096];
			WideCharToMultiByte(CP_UTF8,0,finddata.cFileName,-1,buf,4096,NULL,NULL);
			GURL::UTF8 Entry(buf,*this);
#else
    ...
    else if (fname[0] && fname[1]==colon)
    {
      ...
      {       // Case "x:abcd"
          ...
          || s[1]!=colon)
        {
          ...
          drv[3]=0;
#ifdef UNICODE
			 wchar_t buf_drv[1024],buf_str[1024],*ws;
			 MultiByteToWideChar(CP_UTF8,0,drv,01,buf_drv,1024);
			 GetFullPathName(buf_drv,maxlen,buf_str,&ws);
			 s = string_buffer;
			 WideCharToMultiByte(CP_UTF8,0,buf_str,-1,string_buffer,maxlen,NULL,NULL);
#else
