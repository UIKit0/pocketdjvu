...
#elif defined(WIN32) || defined(OS2) || defined(UNDER_CE)
...
#else
#if 0
	wchar_t buff[MAXPATHLEN]={0};
	GUTF8String str(dirname);
	char *thisbuf = str.getbuf();
	MultiByteToWideChar(CP_UTF8,0,thisbuf,-1,buff,MAXPATHLEN);
	if (wcslen(buff) && _wchdir(buff)==-1)
		G_THROW(errmsg());
	wchar_t drv[2];
	drv[0] = dot; drv[1] = 0;
	wchar_t* result = _wgetcwd(buff,MAXPATHLEN);
	GetFullPathName(drv,MAXPATHLEN,buff,&result);
	char utf8buf[MAXPATHLEN*6]={0};
	int rv = WideCharToMultiByte(CP_UTF8,0,buff,-1,utf8buf,MAXPATHLEN*6,NULL,NULL);
	DWORD err = GetLastError();
	DWORD inb = ERROR_INSUFFICIENT_BUFFER;
	DWORD inv = ERROR_INVALID_FLAGS;
	DWORD inp = ERROR_INVALID_PARAMETER;
	return GUTF8String(utf8buf);
#endif
#endif
